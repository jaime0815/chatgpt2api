...
> **普通用户 Chat 工作台已上线**
>
> - 普通用户登录后默认进入 `/chat`，提供接近 ChatGPT 的多轮对话布局、聊天模型选择与流式输出。
> - 支持停止文本流、重试助手回复，以及编辑用户消息后重新发送；会话可单独删除，也可一键清空当前用户在本浏览器中的本地历史。
> - 聊天内可直接切换到图片生成或编辑模式，复用模型、质量、尺寸/比例等参数；参考图上限为 **10 张**，聊天页面的生成数量支持 **1-100 张**。
> - 聊天记录、模型偏好、图片设置和滚动位置按登录用户的 `subject_id` 存在当前浏览器中；不同用户之间不会混用，也不会同步为服务端聊天记录。

> [!NOTE]
> **使用与验证说明**
>
> - 聊天图片设置会按用户独立保存，不读取旧的全局画图设置。聊天页面的 `1-100` 数量范围仅适用于该交互；OpenAI 兼容图片接口的参数范围以本文下方接口说明为准。
> - 文本聊天附件（包括 PDF、Office 文档）尚未接入真实上游 uploader。页面会提示移除附件，直接提交附件会返回 `attachment_unavailable`。
> - 真实兼容 API 冒烟测试默认跳过。需要验证外部服务时，使用 `RUN_LIVE_COMPAT_API=1` 并通过环境变量提供目标、授权、模型与本地图片夹具；请勿将凭证写入仓库。完整配置见[普通用户聊天](#普通用户聊天)。
