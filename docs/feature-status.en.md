...
| 普通用户文本聊天交互 | ✅  | 提供 ChatGPT 式多轮消息流、模型选择，以及文本流停止、助手回复重试和用户消息编辑后重发。 |
| 普通用户聊天记录隔离 | ✅  | 会话、模型偏好、图片设置和滚动位置仅保存在当前浏览器，并按登录用户 `subject_id` 分区；支持删除单个会话或清空本地历史，不作为服务端聊天历史同步。 |
| 聊天内图片生成 / 编辑 | ✅  | 复用既有画图工作台的模型、质量、尺寸/比例和生成数量设置；聊天图片设置按 `subject_id` 独立保存，不读取旧的全局画图设置；图片参考图最多 10 张，生成数量沿用 `1-100` 范围。 |
| 文档及文本聊天附件上游上传 | ❌  | 默认聊天流尚未接入真实 upstream uploader。携带附件调用 `/api/chat/stream` 会返回 `attachment_unavailable` 错误事件，不能宣称 PDF/Office 或普通文本附件已支持。 |
| PDF / Office 实时附件冒烟 | ⚠️ | 可选 live probe 仍需要外部服务 URL、Bearer 凭证和真实本地夹具；在 uploader 未接入前，附件用例预期 `xfail`，不构成可用性证明。 |
| 兼容 API 实测 | ⚠️ | 默认跳过真实网络请求；仅在设置 `RUN_LIVE_COMPAT_API=1` 并显式提供服务 URL、Bearer 授权、模型和图片夹具后执行。缺少条件时会跳过，不会访问默认上游。 |
