...
class ImageEditsJsonApiTests(unittest.TestCase):
    ...
    def test_image_edit_accepts_remote_json_url(self):
        ...
        with mock.patch("api.image_inputs.requests.get", return_value=fake_response) as request_get:
            response = self.client.post(
                "/v1/images/edits",
                headers=AUTH_HEADERS,
                json={"prompt": "允许远程拉图", "images": [{"image_url": "https://images.example/source.png"}]},
            )

        request_get.assert_called_once()
        self.assertEqual(response.status_code, 200, response.text)
        self.assertEqual(self.calls[0]["images"], [(b"remote-png", "source.png", "image/png")])
